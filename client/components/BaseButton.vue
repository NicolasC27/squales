<template>
    <span class="rounded-md" :class="{
        'block w-full': block,
        'inline-flex': !block,
        'shadow-sm': !variant.includes('link')
    }">
        <button v-bind="$attrs" :type="$attrs.type || 'button'" :disabled="disabled || loading" ref="button"
            ...
            <!-- <loader-icon v-if="loading" class="spin-animation" :class="{
                'h-4 w-4': size === 'xs',
                'h-5 w-5': size === 'sm',
...
<script setup>
// import { LoaderIcon } from "@nuxtclub/feathericons";
